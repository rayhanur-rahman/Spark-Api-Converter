...
std::string Util::getOperandString(std::string id, SymbolTable *symTab, std::string *posString) {
    ...
    std::vector<std::pair<std::string, Variable *> >::iterator locIT = std::find_if(
            symTab->curFunction->localVariables.begin(), symTab->curFunction->localVariables.end(),
            Comparision<std::string, Variable *>(id));
    if (locIT != symTab->curFunction->localVariables.end()) {
        newOperand.append("local[");

        if (posString != NULL) {
            symTab->curFunction->funcStats.push(
                    getAryPosStmt(locIT->second->loc, posString, symTab->curFunction->localVarCnt));
            newOperand.append("local[");
        ...
        std::vector<std::pair<std::string, Variable *> >::iterator argIT = std::find_if(
                symTab->curFunction->arguments.begin(), symTab->curFunction->arguments.end(),
                Comparision<std::string, Variable *>(id));
        if (argIT != symTab->curFunction->arguments.end()) {
            ...
            if (posString != NULL) {
                symTab->curFunction->funcStats.push(
                        getAryPosStmt(argIT->second->loc, posString, symTab->curFunction->localVarCnt));
                newOperand.append("local[");
                newOperand.append(Util::to_string(symTab->curFunction->localVarCnt++));
                newOperand.push_back(']');
            } else {
            ...
            std::vector<std::pair<std::string, Variable *> >::iterator glbIT = std::find_if(
                    symTab->globalVariables.begin(), symTab->globalVariables.end(),
                    Comparision<std::string, Variable *>(id));
            if (glbIT != symTab->globalVariables.end()) {
                newOperand.append("global[");
                if (posString != NULL) {
                    symTab->curFunction->funcStats.push(
                            getAryPosStmt(glbIT->second->loc, posString, symTab->curFunction->localVarCnt));
                    newOperand.append("local[");
                    newOperand.append(Util::to_string(symTab->curFunction->localVarCnt++));
                    newOperand.push_back(']');
                } else {
