...
public:
    ...
    Function(std::string newDatatype, std::string newName)
            : datatype(newDatatype), name(newName), localVarCnt(0), isDeclaration(true) {}

    ~Function() {
        for (std::vector<std::pair<std::string, Variable *> >::iterator it = arguments.begin();
             it != arguments.end(); ++it) {
            delete it->second;
        }

        for (std::vector<std::pair<std::string, Variable *> >::iterator it = localVariables.begin();
             it != localVariables.end(); ++it) {
            delete it->second;
        }
    }
