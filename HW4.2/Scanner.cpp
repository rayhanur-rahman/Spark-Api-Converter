...
void Scanner::populateToken() {
    while (lineIt != curLine.end()) {
        if (curToken->getID() == Token::IDTYPE_METASTATEMENT) {
            ...
            {
                if (Util::isMetaChar(
                        lineIt))                /*If the symbol is a meta character, change the token to meta character*/
                {
                    curToken->setID(Token::IDTYPE_METASTATEMENT);
                    curToken->push(*lineIt);
                } else {
                    curToken->setID(
                            Token::IDTYPE_SYMBOL);            /*If the symbol is a not meta character, initialize the token to symbol*/
                    curToken->push(*lineIt);
                    if (Util::isDoubleSymbol(
                            lineIt).first)        /*If the symbol consists of 2 parts then 2 positions should be moved forward*/
                    {
                    ...
                    break;
                       ...
                       Util::isReservedWord(curToken->getTokenName())) {
        ...
        } else {                                    /* This branch will execute when the character is not a digit,
