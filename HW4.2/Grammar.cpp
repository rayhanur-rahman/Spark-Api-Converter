...
bool Grammar::paramList(std::vector<std::pair<std::string, Variable *> > &arguments) {
    ...
    */
    ...
    if (parse->curToken()
        ...
        if (parse->curToken()
            && parse->curToken()->getID() == Token::IDTYPE_IDENTIFIER) {
            arguments.push_back(std::pair<std::string, Variable *>(parse->curToken()->getTokenName(),
                                                                   new Variable(datatype,
                                                                                parse->curToken()->getTokenName())));
            parse->nextToken();
...
bool Grammar::paramListPrime(std::vector<std::pair<std::string, Variable *> > &arguments) {
    ...
    */
    ...
    if (parse->curToken()
        && parse->curToken()->getID() == Token::IDTYPE_IDENTIFIER) {
        arguments.push_back(std::pair<std::string, Variable *>(parse->curToken()->getTokenName(), new Variable("void",
                                                                                                               parse->curToken()->getTokenName())));
        parse->nextToken();
        return nonEmptyListPrime(arguments);
    }
...
bool Grammar::idPrime(std::string dataType, std::string idName) {
    ...
    */
    ...
    if (parse->curToken()
        ...
        if (expression(&root)
            ...
            if (parse->getSymbolTable()->curFunction != NULL) {
                parse->getSymbolTable()->curFunction->localVariables.push_back(
                        std::pair<std::string, Variable *>(idName, new Variable(dataType, idName,
                                                                                parse->getSymbolTable()->curFunction->localVarCnt,
                                                                                Variable::ARRAY,
                                                                                std::atoi(root->value.c_str()))));
                parse->getSymbolTable()->curFunction->localVarCnt += std::atoi(root->value.c_str());
            } else {
                parse->getSymbolTable()->globalVariables.push_back(std::pair<std::string, Variable *>(idName,
                                                                                                      new Variable(
                                                                                                              dataType,
                                                                                                              idName,
    ...
    if (parse->getSymbolTable()->curFunction != NULL) {
        parse->getSymbolTable()->curFunction->localVariables.push_back(std::pair<std::string, Variable *>(idName,
                                                                                                          new Variable(
                                                                                                                  dataType,
                                                                                                                  idName,
                                                                                                                  parse->getSymbolTable()->curFunction->localVarCnt,
    ...
    */
    ...
    if (idUse(idName, assignStmt)) {
        if (parse->curToken()
            ...
            parse->nextToken();
            expression(&root);
            parse->evaluateASTTree(root, false, true);
            if (parse->curToken()
                && parse->curToken()->getSymType() == Token::SYMTYPE_SEMICOLON) {
                assignStmt.append(" = ");
    ...
    */
    ...
    if (parse->curToken() && parse->curToken()->getSymType() == Token::SYMTYPE_LEFT_BRACKET) {
        ...
        if (expression(&aryPos)
            ...
            *root = new ASTNode(Util::getOperandString(idName, parse->getSymbolTable(), &(aryPos->value)),
                                ASTNode::OPERAND);
            delete aryPos;
            parse->nextToken();
            return true;
